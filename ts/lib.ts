...
interface ModifierStatePayload {
    modifier: ModifierKey;
    active: boolean;
    timestamp: number;
}

type ModifierKey = "shift" | "control" | "alt";
type PencilSqueezeModifier = ModifierKey | "off";
...
let connectionOverlay: HTMLDivElement;
let connectionTitle: HTMLHeadingElement;
let connectionStateText: HTMLParagraphElement;
let connectionDetailText: HTMLParagraphElement;
let connectionAttemptText: HTMLSpanElement;
let connectionHostText: HTMLSpanElement;
let monitorSelect: HTMLSelectElement;
let pressurePreview: HTMLCanvasElement;
let pressurePreviewCtx: CanvasRenderingContext2D;
let pencilSqueezeSelect: HTMLSelectElement;
...
let pressureReadoutStart: HTMLSpanElement;
let pressureReadoutMiddle: HTMLSpanElement;
let pressureReadoutEnd: HTMLSpanElement;
...
let pressurePreviewMetrics: PressurePreviewMetrics = {
    width: 256,
    height: 176,
    dpr: 1,
};
...
const BINARY_EVENT_TYPE_MAP: Record<string, number> = {
    "pointerdown": 0,
    "pointerup": 1,
    "pointercancel": 2,
    "pointermove": 3,
...
let pencilSqueezeModifier: PencilSqueezeModifier = "off";
let penIsInRange = false;
let penSqueezeSignalActive = false;
let activePencilSqueezeModifier: ModifierKey | null = null;
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    connectionOverlay = document.getElementById("connection-overlay") as HTMLDivElement;
    connectionTitle = document.getElementById("connection-title") as HTMLHeadingElement;
    connectionStateText = document.getElementById("connection-state") as HTMLParagraphElement;
    connectionDetailText = document.getElementById("connection-detail") as HTMLParagraphElement;
    connectionAttemptText = document.getElementById("connection-attempt") as HTMLSpanElement;
    connectionHostText = document.getElementById("connection-host") as HTMLSpanElement;
...
function normalizePencilSqueezeModifier(value: unknown): PencilSqueezeModifier {
    return value === "shift" || value === "control" || value === "alt" ? value : "off";
}

function loadPencilSqueezeModifier(): PencilSqueezeModifier {
    try {
        return normalizePencilSqueezeModifier(
            window.localStorage.getItem(PENCIL_SQUEEZE_STORAGE_KEY)
        );
    } catch {
        return "off";
    }
...
function updatePencilSqueezeModifierBinding(timeStamp: number) {
    const nextModifier: ModifierKey | null =
        penSqueezeSignalActive && pencilSqueezeModifier !== "off" ? pencilSqueezeModifier : null;
...
function setPencilSqueezeModifier(nextModifier: PencilSqueezeModifier, persist: boolean) {
...
function formatCurvePercent(value: number) {
...
function resizePressurePreviewCanvas(force: boolean = false) {
...
function buildPressureCurvePreviewPaths(layout: ReturnType<typeof getPressurePreviewLayout>) {
...
function drawPressureCurveBubble(
    text: string,
    anchorX: number,
    anchorY: number,
    layout: ReturnType<typeof getPressurePreviewLayout>,
    background: string,
    border: string,
    textColor: string
) {
...
function getPressureHandleTooltip(handle: PressureHandle) {
    if (handle === "start") {
...
function drawPressureCurvePreview() {
    ...
    const glowGradient = pressurePreviewCtx.createRadialGradient(
        width * 0.24,
        height * 0.12,
        0,
        width * 0.24,
    ...
    if (livePressureIndicator.visible) {
        ...
        drawPressureCurveBubble(
            `${formatCurvePercent(livePressureIndicator.raw)} -> ${formatCurvePercent(livePressureIndicator.mapped)}`,
            liveX,
            liveY,
            layout,
            "rgba(7, 17, 22, 0.94)",
    ...
    if (activePressureHandle) {
        ...
        drawPressureCurveBubble(
            getPressureHandleTooltip(activePressureHandle),
            point.x,
            point.y,
            layout,
            "rgba(23, 16, 9, 0.94)",
            "rgba(240, 180, 92, 0.34)",
            "#fff3db"
        );
    }
...
function onPenPointerRawUpdate(event: Event) {
...
function getPointerTangentialPressure(e: PointerEvent) {
    const tangentialPressure = (e as PointerEvent & { tangentialPressure?: number })
        .tangentialPressure;
    return typeof tangentialPressure === "number" ? Math.abs(tangentialPressure) : 0;
}

function isPenBarrelButtonActive(e: PointerEvent) {
    return (e.buttons & 0x02) !== 0;
}

function shouldConsumePenBarrelEvent(e: PointerEvent) {
    return pencilSqueezeModifier !== "off" && (isPenBarrelButtonActive(e) || e.button === 2);
}

function getPenSqueezeSignalState(e: PointerEvent) {
    return (
        isPenBarrelButtonActive(e) ||
        getPointerTangentialPressure(e) >= PEN_SQUEEZE_TANGENTIAL_PRESSURE_THRESHOLD
    );
}

function syncPenSqueezeModifierState(e: PointerEvent) {
    const nextSignalActive =
        e.type === "pointerleave" || e.type === "pointercancel"
            ? false
...
function buildPenPointerPayload(e: PointerEvent, eventType: string): JsonPointerEvent | null {
    ...
    const squeezeOnlyHoverTransition =
        consumeBarrelEvent &&
        e.button === 2 &&
    ...
    if (rawBtn === 0) btn = 1;
    else if (rawBtn === 2 && !consumeBarrelEvent) btn = 2;
...
function sendRelativeMouseMove(dx: number, dy: number, timeStamp: number): boolean {
    if (dx === 0 && dy === 0) {
        return true;
    }
...
function sendWheelEvent(dx: number, dy: number, timeStamp: number): boolean {
    if (dx === 0 && dy === 0) {
        return true;
    }
...
function sendZoomEvent(delta: number, timeStamp: number): boolean {
    if (delta === 0) {
        return true;
    }
...
function sendModifierState(modifier: ModifierKey, active: boolean, timeStamp: number) {
    const msg: { ModifierState: ModifierStatePayload } = {
        ModifierState: {
            modifier,
            active,
            timestamp: Math.round(timeStamp * 1000),
...
function writeBinaryPointerEvent(view: DataView, offset: number, e: JsonPointerEvent) {
...
type ConnectionVisualState = "connecting" | "syncing" | "reconnecting" | "offline" | "ready";

function sendJsonMessage(value: unknown, allowWhenCongested = true): boolean {
    if (!ws || ws.readyState !== WebSocket.OPEN) {
        return false;
    }
...
function setConnectionVisualState(state: ConnectionVisualState, detail?: string) {
...
function setupConnectionLifecycle() {
    window.addEventListener("online", () => {
        ...
        if (ws.readyState === WebSocket.OPEN) {
            if (performance.now() - lastServerActivityAt > WS_VISIBLE_STALE_GRACE_MS) {
                setConnectionVisualState(
                    "reconnecting",
                    "Network is back. Restoring the tablet session..."
                );
                ws.close();
            } else {
    ...
    window.addEventListener("offline", () => {
        setConnectionVisualState(
            "offline",
            "The tablet lost network access. Waiting for the local network to come back..."
        );
    });
    document.addEventListener("visibilitychange", () => {
        ...
        if (
            ...
            setConnectionVisualState(
                "reconnecting",
                "Checking the connection after returning to the tablet..."
            );
            ws.close();
...
function scheduleReconnect() {
    ...
    const delay = Math.min(
        WS_RECONNECT_BASE_DELAY_MS * Math.pow(1.5, reconnectAttempt),
...
function loadSelectedMonitorSelection(): string | null {
    try {
...
function rememberSelectedMonitor(id: string | null) {
...
function setupUI() {
    ...
    pencilSqueezeSelect = document.getElementById("pencil-squeeze-modifier") as HTMLSelectElement;
    ...
    pressureReadoutStart = document.getElementById("pressure-readout-start") as HTMLSpanElement;
    pressureReadoutMiddle = document.getElementById("pressure-readout-middle") as HTMLSpanElement;
    pressureReadoutEnd = document.getElementById("pressure-readout-end") as HTMLSpanElement;
    ...
    pencilSqueezeSelect.addEventListener("change", () => {
        setPencilSqueezeModifier(
            normalizePencilSqueezeModifier(pencilSqueezeSelect.value),
            true
        );
    });
